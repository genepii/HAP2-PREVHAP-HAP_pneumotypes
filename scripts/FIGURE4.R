...
# Read your dataset
data <- read.table("data/data_for_bubbles_examples.txt", sep="\t", header=TRUE, quote="", comment.char="", stringsAsFactors = FALSE)
...
library(vegan)
data1<-read.delim("data/virome.txt", row.names = 1, check.names = F)
...
set.seed(1)
data1 <- read.delim("data/virome.txt", row.names = 1)
dataTransposed1 <- t(data1)
...
# Load data
otu = read.delim(file="data/virome.txt", header=T, row.names=1, check.names=F)
...
pdf("Volcano_virsig.pdf",width=8,height=8)
volcano
dev.off()
...
# Load data
expr <- read.delim("data/active_or_virome_fucntions.txt", header = TRUE, row.names = 1, check.names = FALSE)
...
# Load your data
Bac <- read.delim("virome.txt", header = TRUE, stringsAsFactors = FALSE, row.names = 1)
VAL <- read.delim("virome.txt", header = TRUE, stringsAsFactors = FALSE, check.names = FALSE)
...
# Load data
otu = read.delim("virome.txt", header=T, row.names=1, check.names=F)
